...
A common question that gets asked with regards to Red Hat Lightspeed and the recommendations that it offers is how can I see the list of recommendations?  For example, are there recommendations for a database like SAP? +
...
In other words, show me issues that affect my systems that are enabled.
...
1. Locate the filters and remove the two default filters. +
In the image below there are 1951 total recommendations listed, though the count of total recommendations may vary as new recommendations are regularly released. +
...
[start=2]
2. In the `Filter by name` box type “SAP” and press enter. +
