...
= Red Hat Lightspeed via the Hybrid Cloud Console - Evaluate regulatory compliance.
...
You’ll then review the results and identify any changes that need to be made to bring the system into compliance with the policy.
...
[%collapsible]
====
If you still have the Hybrid Cloud Console open from the previous module, skip to step #2 in the next section.
...
5. Near the `Policy type` box in the `Filter by policy name` search bar, type *CIS*
...
NOTE: Please replace the default policy name with  the policy name as shown above. This helps you find your policy in later steps and will help with automated cleanup tasks.  
...
15. Next to the `Name` filter in the `Filter by name` box enter: *Set Existing Passwords Maximum Age*
16. Expand the resulting rule so that you can see the details. +
Scroll down to where you see the `Depends on values` as shown in the image below.
...
NOTE: It is suggested to leave your Hybrid Cloud Console tab or window open as you will need it in a future section.
...
This RHEL 9.7 system was built using the image builder service in Red Hat Lightspeed. + 
As a part of the image build process Infinicorp identified that the system needed to be compliant to CIS Level 1 Server.  +
As a result it is expected that this system will be compliant, but there might still be improvements that can be made to the image. + 
...
1. From your showroom environment, click the tab labeled `rhel-{guid}-1 terminal`.

.rhel-{guid}-1 terminal tab
image::lb1187_M3_terminaltab.png[link=self, window=blank, width=100%]
...
If you closed the page, click this https://console.redhat.com/insights/compliance/reports[link] to go to the right place.

2. Locate (do not click) your policy. It may help to use the filter at the top to locate your policy. +
Your policy should be named `{guid} CIS L1 Server Policy`.

NOTE: If you did not run `insights-client --compliance` in the previous section, or if the operation is still in progress, your policy will not display here.  Policies do not display reports until at least one system is reporting.
...
From here you can see your policy status. + 
Starting in the upper left, we have 100% of systems compliant to the policy.  +
The only conditions that are showing are compliant and non-compliant, but there are several possible conditions: +

* `Compliant` - Passing with the specified threshold percentage
* `Non-compliant` - Not passing with the specified threshold percentage
...
However, these conditions are great to be aware of. + 
It is common for someone for example to `dnf upgrade` and update the ssg package to the latest version. + 
That latest version may not be compatible with an older version of RHEL and the results can be deceptive. + 
Rather than giving a deceptive or potentially incorrect score, the compliance report will report as unsupported while still giving you a rules failed number. +

`Not reporting` is also a huge help in large environments - If you have 5000 systems that you expect to be held to the CIS policy and only 4990 are reporting it might otherwise be difficult to know that those last 10 systems aren’t being scanned.  +
The compliance report will tell you if a system isn’t reporting.  +
There are also tabs below for `Reporting` and `Never reported` where you can easily get a list of these systems. 
...
An interactive demo was shared at the end of module 2 that shows how this works. +
...
The playbook is downloaded via your browser.  

NOTE: The exact steps may vary per browser, in chrome you click save to save the file. 
